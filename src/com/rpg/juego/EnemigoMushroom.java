...
public class EnemigoMushroom extends EnemigoBase {
    ...
    // Aumento de fuerza para que el hongo salte más lejos
    private double velocidadSalto = 5.5;
    ...
    public void iniciarMarchaCinematica(int metaX) {
        ...
        this.mirandoDerecha = false; // Van a la izq
    }
    ...
    public void actualizarIA(Jugador jugador) {
        ...
        // Ajustes para que te ataque desde más lejos y camine más rápido
        int rangoAtaqueX = 180;
        int margenY = 40;
        float velocidad = 2.2f;
        ...
        else if (distX < 750 && estadoActual != ATTACK) { // Ahora te ve de más lejos
            estadoActual = RUN;
        ...
        if (estadoActual == ATTACK) {
            if (aniIndex < 4) {
                // Se queda quieto unos frames preparándose para saltar
            } else if (aniIndex == 4 && !saltoCalculado) {
                saltoCalculado = true;
                GestorSonidos.reproducir(GestorSonidos.MUSH_SALTO);
                enElAire = true;
                timerSalto = 26; // Tiempo que pasa volando hacia ti
