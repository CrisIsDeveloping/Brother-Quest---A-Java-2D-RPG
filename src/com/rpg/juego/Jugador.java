...
public class Jugador extends Entidad {
    ...
    private long ultimoUsoHabilidad = 0;
    private final long COOLDOWN_HABILIDAD = 7000;
    ...
    private final long COOLDOWN_TIEMPO = 5000; // Cooldown bajado a 5 segundos (antes 10)
    ...
    private int nivel = 1, xp = 0, xpParaSiguienteNivel = 100;

    public Jugador() {
        super();
        this.x = 100;
        this.y = 490;
        this.hitbox = new CajaColision(0, 0, anchoHitbox, altoHitbox, 0, -offsetHitboxY);
        this.vidaMax = 100;
        this.vida = 100;
        this.dano = 25;
    }
    ...
    public void mover(boolean w, boolean s, boolean a, boolean d) {
        if (estado == Estado.MUERTO || rodando)
            return;
        ...
        // Reducimos la velocidad vertical un 40% segun lo solicitado (0.6f)
        int vy = (int) (velocidad * factorVelocidad * 0.6f);
        ...
        if (y > limiteAbajo)
            y = limiteAbajo;
        if (x < 0 && GamePanel.getInstancia().estadoActual != GamePanel.ESTADO_CINEMATICA_BOSS)
            x = 0;
    }

    public int getXLogical() { return (int) x; }
    public int getYLogical() { return (int) y; }
    ...
    public void pararMovimiento() {
        ...
        this.moving = false;
        // SOLO reseteamos la animación si no estamos haciendo una acción especial
        // Esto evita que al soltar una tecla de movimiento se corte el ataque, rodada o poder
        if (!atacando && !rodando && !lanzandoPoder && playerAction != HIT && playerAction != DEAD) {
            this.aniIndex = 0;
            this.aniTick = 0;
        }
    ...
    public void saltar() {
        ...
        if (!enAire && !rodando && estado != Estado.MUERTO && !atacando && !defendiendo) {
            if (stamina >= COSTO_STAMINA_SALTO) {
                ...
                GestorSonidos.reproducirSaltoAleatorio();
            }
        }
    }

    public void rodar() {
        if (atacando || lanzandoPoder || defendiendo)
            return;
    ...
    private void actualizarStamina() {
        ...
        if (defendiendo) {
            ...
            if (escudoRoto) {
                if (tiempoActual - tiempoSinCorrer > 1500)
                    stamina += 0.5f;
    ...
    private void actualizarAnimacionTick() {
        aniTick++;
        // ROLL y ATTACK tienen velocidades propias más rápidas
        int velocidadAnimacionActual;
        if (playerAction == ROLL) {
            velocidadAnimacionActual = 3;
        } else if (playerAction == ATTACK) {
            velocidadAnimacionActual = 3; // Espada rápida (era aniSpeed=6, ahora 3 = doble de rápido)
        } else if (playerAction == MAGIC_ATTACK) {
            velocidadAnimacionActual = 4;
        } else {
            velocidadAnimacionActual = aniSpeed;
        }
    ...
    private void setAnimacion() {
        ...
        if (estado == Estado.MUERTO)
            ...
            playerAction = ROLL;
        else if (lanzandoPoder)
            playerAction = MAGIC_ATTACK;
        else if (atacando)
            playerAction = ATTACK;
        else if (defendiendo)
            playerAction = BLOCK;
        ...
        // --- Gestión de sonido de correr centralizada ---
        if (playerAction == RUN && !enAire && estado != Estado.MUERTO) {
            GestorSonidos.reproducirenLoop(GestorSonidos.RUN_SONIDO);
        } else {
            GestorSonidos.detener(GestorSonidos.RUN_SONIDO);
        }
        ...
        if (startAni != playerAction) {
            ...
            if (playerAction == ATTACK)
                GestorSonidos.reproducirEspadaAleatoria();
        }
    }

    public void recibirGolpe(int d) {
        if (rodando || estado == Estado.MUERTO || invencible)
            return;
        if (!defendiendo) {
            vida -= d;
            GestorSonidos.reproducirHurtAleatorio();
            if (vida <= 0) {
                vida = 0;
    ...
    public boolean isEscudoRoto() {
        return escudoRoto;
    }

    public boolean isLanzandoPoder() {
        return lanzandoPoder;
    }
    ...
    public long getUltimoUsoHabilidad() {
        return ultimoUsoHabilidad;
    }

    public long getCooldownHabilidad() {
        return COOLDOWN_HABILIDAD;
    }

    public int getPocionSeleccionada() {
    ...
    public int getPocionesFuerza() {
        return pocionesFuerza;
    }

    public int getPocionesVelocidad() {
        return pocionesVelocidad;
    }

    public long getUltimoUsoVida() {
        return ultimoUsoVida;
    }

    public long getUltimoUsoFuerza() {
        return ultimoUsoFuerza;
    }

    public long getUltimoUsoVelocidad() {
        return ultimoUsoVelocidad;
    }
    ...
    public void setLanzandoPoder(boolean b) {
        this.lanzandoPoder = b;
    }

    public void setVida(int v) {
        this.vida = Math.min(vidaMax, v);
    }

    public void setVidaMax(int vm) {
        this.vidaMax = vm;
        this.vida = vm;
    }
    ...
    public void ganarXP(int c) {
        ...
        while (xp >= xpParaSiguienteNivel) {
            ...
            nivel++;
            // Subida de nivel: +50% de las stats actuales (escala multiplicativa)
            int vidaGanada = Math.max(20, (int)(vidaMax * 0.50f));
            int danoGanado = Math.max(3,  (int)(dano   * 0.50f));
            vidaMax += vidaGanada;
            dano    += danoGanado;
            vida = vidaMax; // Curación completa al subir de nivel
            xpParaSiguienteNivel = (int)(xpParaSiguienteNivel * 1.4);
            dispararDestello(new Color(255, 215, 0)); // Destello dorado al subir de nivel
        }
    }

    public void cambiarPocion(int dir) {
        pocionSeleccionada = (pocionSeleccionada + dir + 3) % 3;
    }
    ...
    public void usarPocionVida() {
        if (pocionesVida > 0 && vida < vidaMax && (System.currentTimeMillis() - ultimoUsoVida) >= COOLDOWN_TIEMPO) {
            pocionesVida--;
            vida = Math.min(vidaMax, vida + (int)(vidaMax * 0.40f)); // Cura el 40% de la vida máxima
            dispararDestello(Color.GREEN);
            ultimoUsoVida = System.currentTimeMillis();
        }
    }
    ...
    public void usarPocionFuerza() {
        if (pocionesFuerza > 0 && (System.currentTimeMillis() - ultimoUsoFuerza) >= COOLDOWN_TIEMPO) {
            pocionesFuerza--;
            tiempoFuerza = 600;
            ultimoUsoFuerza = System.currentTimeMillis();
            dispararDestello(new Color(180, 50, 255));
        }
    }

    public void usarPocionVelocidad() {
        if (pocionesVelocidad > 0 && !velocidadActiva
                && (System.currentTimeMillis() - ultimoUsoVelocidad) >= COOLDOWN_TIEMPO) {
            pocionesVelocidad--;
            velocidadActiva = true;
            ultimoUsoVelocidad = System.currentTimeMillis();
            dispararDestello(Color.CYAN);
            float vOrig = 4.0f;
            this.velocidad = vOrig * 1.7f;
            Timer t = new Timer(5000, e -> {
                this.velocidad = vOrig;
                this.velocidadActiva = false;
            });
            t.setRepeats(false);
            t.start();
        }
    ...
    public boolean usarHabilidad() {
        long tiempoActual = System.currentTimeMillis();
        if (tiempoActual - ultimoUsoHabilidad >= COOLDOWN_HABILIDAD) {
    ...
    public void dispararDestello(Color color) {
        this.colorDestello = color;
        this.alfaDestello = 0.8f;
    }
    ...
    public Rectangle getAttackBox() {
        // Evaluacion del "Active Frame" correspondiente al impacto logico de la
        // animacion
    ...
    public void dibujar(Graphics g, int cameraX) {
        ...
        if (img != null) {
            if (mirandoDerecha)
                g.drawImage(img, xPantalla, yPantalla, ANCHO_VISUAL, ALTO_VISUAL, null);
            else
                g.drawImage(img, xPantalla + ANCHO_VISUAL, yPantalla, -ANCHO_VISUAL, ALTO_VISUAL, null);
        }
    ...
    public void setVelocidadCinematica(float v) {
        this.velocidad = v;
    }

    public void setPocionesVida(int p) { this.pocionesVida = p; }
    public void setPocionesFuerza(int p) { this.pocionesFuerza = p; }
    public void setPocionesVelocidad(int p) { this.pocionesVelocidad = p; }
}